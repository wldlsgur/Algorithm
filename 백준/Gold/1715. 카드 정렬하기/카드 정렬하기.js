class MinHeap {
  ...
  insertNode(node) {
    ...
    while (
      currentIdx > 1 &&
      this.heap[this.getParentIndex(currentIdx)] > this.heap[currentIdx]
    ) {
  ...
  deleteNode() {
    ...
    while (true) {
      ...
      if (rightChildIdx < this.heap.length) {
        ...
        // 왼쪽 자식만 있는 경우 작은 값은 반드시 왼쪽
        smallIdx = leftChildIdx;
      }

      if (this.heap[currentIdx] > this.heap[smallIdx]) {
        // 루트 노드와 작은 값의 자식 노드를 비교
...
rl.on("line", (input) => {
  ...
  const sizes = inputs.slice(1, inputs.length).map(Number);
...
function solution(N, sizes) {
  ...
  sizes.forEach((element) => minHeap.insertNode(element));
  for(let i=0 ; i<N-1 ; i++) {
    const card1 = minHeap.deleteNode();
    const card2 = minHeap.deleteNode();
    const addCard = card1 + card2;

    result += addCard;
    minHeap.insertNode(addCard);
  }
